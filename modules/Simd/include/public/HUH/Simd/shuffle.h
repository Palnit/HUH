...
HUH_FORCE_INLINE Register<float, 4> ShuffleRegister(
    const Register<float, 4>& reg) noexcept {
    return _mm_shuffle_ps(reg, reg, HUH_SHUFFLE_MASK(X, Y, Z, W));
}

template<>
HUH_FORCE_INLINE Register<float, 4> ShuffleRegister<0, 1, 2, 3>(
    const Register<float, 4>& reg) noexcept {
    return reg;
}

template<>
HUH_FORCE_INLINE Register<float, 4> ShuffleRegister<0, 1, 0, 1>(
    const Register<float, 4>& reg) noexcept {
    return _mm_movelh_ps(reg, reg);
}

template<>
HUH_FORCE_INLINE Register<float, 4> ShuffleRegister<2, 3, 2, 3>(
    const Register<float, 4>& reg) noexcept {
    return _mm_movehl_ps(reg, reg);
}

template<>
HUH_FORCE_INLINE Register<float, 4> ShuffleRegister<0, 0, 1, 1>(
    const Register<float, 4>& reg) noexcept {
    return _mm_unpacklo_ps(reg, reg);
}

template<>
HUH_FORCE_INLINE Register<float, 4> ShuffleRegister<2, 2, 3, 3>(
    const Register<float, 4>& reg) noexcept {
...
template<>
HUH_FORCE_INLINE Register<float, 4> ShuffleRegister<0, 0, 2, 2>(
    const Register<float, 4>& reg) noexcept {
    return _mm_moveldup_ps(reg);
}

template<>
HUH_FORCE_INLINE Register<float, 4> ShuffleRegister<1, 1, 3, 3>(
    const Register<float, 4>& reg) noexcept {
...
template<>
HUH_FORCE_INLINE Register<float, 4> ShuffleRegister<0, 0, 0, 0>(
    const Register<float, 4>& reg) noexcept {
