...
public:
    ...
    HUH_FORCE_INLINE explicit Register(const float v) : data(_mm_load1_ps(&v)) {}
    HUH_FORCE_INLINE explicit Register(const float* p) : data(_mm_loadu_ps(p)) {}
    ...
    HUH_FORCE_INLINE friend Register operator+(const Register& lhs, const Register& rhs) noexcept {
    ...
    HUH_FORCE_INLINE friend Register operator-(Register lhs, const Register& rhs) noexcept {
        lhs.data = _mm_sub_ps(lhs.data, rhs.data);
        return lhs;
    }

    HUH_FORCE_INLINE Register operator-() const noexcept { return {_mm_sub_ps(_mm_setzero_ps(), data)}; }
    ...
    HUH_FORCE_INLINE friend Register operator*(Register lhs, const float& rhs) noexcept {
