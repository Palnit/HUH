...
public:
    ...
    HUH_CONSTEXPR_FORCE Vector& operator=(Vector<T2, 2>&& other) noexcept {
    ...
    template<HUH::ConvertableFrom<T> T2>
    HUH_CONSTEXPR_FORCE Vector& operator=(const Vector<T2, 2>& other) noexcept {
        data[0] = static_cast<ValueType>(other[0]);
        data[1] = static_cast<ValueType>(other[1]);
        return *this;
    }
    ...
    HUH_CONSTEXPR_FORCE Vector(const T (&t)[size]) noexcept : data{t[0], t[1]} {}
    ...
    HUH_NODISCARD HUH_CONSTEXPR_FORCE const T* UV() const noexcept { return data; }
    ...
    template<typename T2>
    HUH_CONSTEXPR_FORCE Vector& operator+=(const Vector<T2, 2>& rhs) noexcept {
        data[0] += rhs.data[0];
        data[1] += rhs.data[1];
        return *this;
    }
    ...
    template<typename T2>
    HUH_CONSTEXPR_FORCE auto operator+(const Vector<T2, 2>& rhs) const noexcept {
