...
public:
    ...
    HUH_CONSTEXPR_FORCE Vector(const T (&t)[size]) noexcept : data{t[0], t[1], t[2], t[3]} {}
    ...
    HUH_NODISCARD HUH_CONSTEXPR_FORCE const T& W() const noexcept { return data[3]; }
    ...
    HUH_NODISCARD HUH_CONSTEXPR_FORCE const T& A() const noexcept { return data[3]; }

    HUH_NODISCARD HUH_CONSTEXPR_FORCE T* XYZW() noexcept { return data; }
    HUH_NODISCARD HUH_CONSTEXPR_FORCE const T* XYZW() const noexcept { return data; }

    HUH_NODISCARD HUH_CONSTEXPR_FORCE T* RGBA() noexcept { return data; }
    HUH_NODISCARD HUH_CONSTEXPR_FORCE const T* RGBA() const noexcept { return data; }
    ...
    HUH_CONSTEXPR_FORCE auto operator+(const T2& rhs) noexcept {
        Vector<std::common_type_t<T, T2>, 4> result;
        result.data[0] = data[0] + rhs;
        result.data[1] = data[1] + rhs;
        result.data[2] = data[2] + rhs;
        result.data[3] = data[3] + rhs;
        return result;
    }

    template<typename T2>
    HUH_CONSTEXPR_FORCE Vector& operator+=(const Vector<T2, 4>& rhs) noexcept {
        data[0] += rhs.data[0];
        data[1] += rhs.data[1];
        data[2] += rhs.data[2];
        data[3] += rhs.data[3];
    ...
    template<typename T2>
    HUH_CONSTEXPR_FORCE auto operator+(const Vector<T2, 4>& rhs) noexcept {
        Vector<std::common_type_t<T, T2>, 4> result;
