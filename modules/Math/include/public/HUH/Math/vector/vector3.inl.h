...
public:
    ...
    HUH_HOST HUH_DEVICE HUH_CONSTEXPR_FORCE Vector& operator=(Vector<T2, 3>&& other) noexcept {
    ...
    template<HUH::ConvertableFrom<T> T2>
    HUH_HOST HUH_DEVICE HUH_CONSTEXPR_FORCE Vector& operator=(const Vector<T2, 3>& other) noexcept {
        data[0] = static_cast<ValueType>(other[0]);
        data[1] = static_cast<ValueType>(other[1]);
        data[2] = static_cast<ValueType>(other[2]);
        return *this;
    ...
    HUH_HOST HUH_DEVICE HUH_CONSTEXPR_FORCE Vector(const T& x, const T& y, const T& z) noexcept : data{x, y, z} {}
    HUH_HOST HUH_DEVICE HUH_CONSTEXPR_FORCE Vector(const T& v) noexcept : data{v, v, v} {}

    template<HUH::ConvertableFrom<T> T2>
    HUH_HOST HUH_DEVICE HUH_CONSTEXPR_FORCE Vector(const Vector<T2, 3>& other) noexcept
    ...
    HUH_HOST HUH_DEVICE HUH_CONSTEXPR_FORCE Vector(const T (&t)[size]) noexcept : data{t[0], t[1], t[2]} {}
    ...
    HUH_NODISCARD HUH_HOST HUH_DEVICE HUH_CONSTEXPR_FORCE T* XYZ() noexcept { return data; }
    HUH_NODISCARD HUH_HOST HUH_DEVICE HUH_CONSTEXPR_FORCE const T* XYZ() const noexcept { return data; }

    HUH_NODISCARD HUH_HOST HUH_DEVICE HUH_CONSTEXPR_FORCE static std::size_t Size() { return 3; }
    ...
    template<typename T2>
    HUH_HOST HUH_DEVICE HUH_CONSTEXPR_FORCE Vector& operator+=(const Vector<T2, 3>& rhs) noexcept {
        data[0] += rhs.data[0];
        data[1] += rhs.data[1];
        data[2] += rhs.data[2];
        return *this;
    ...
    template<typename T2>
    HUH_HOST HUH_DEVICE HUH_CONSTEXPR_FORCE auto operator+(const Vector<T2, 3>& rhs) const noexcept {
        Vector<std::common_type_t<T, T2>, 3> result{data[0] + rhs.data[0], data[1] + rhs.data[1],
                                                    data[2] + rhs.data[2]};
        return result;
    }

    HUH_HOST HUH_DEVICE HUH_CONSTEXPR_FORCE Vector& operator-() noexcept {
        data[0] = -data[0];
        data[1] = -data[1];
        data[2] = -data[2];
        return *this;
