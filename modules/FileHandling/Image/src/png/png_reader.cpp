...
std::vector<HUH::Uint8> PngReader::ReadPng() {
    ...
    png_get_IHDR(png_ptr, info_ptr, &m_size.data[0], &m_size.data[1], &m_bit_depth, &m_color_type, nullptr, nullptr,
                 nullptr);
    m_channels = png_get_channels(png_ptr, info_ptr);
    HUH_ILOG(LogFileHandlingImage, "Png File MetaData: Size: {} BitDepth: {} ColorType: {} Channels: {}", m_size,
             m_bit_depth, m_color_type, m_channels);
    ...
    m_row_pointers = static_cast<png_bytep*>(png_malloc(png_ptr, m_size.Height() * sizeof(png_bytep)));

    m_pixels.resize(m_size.Height() * m_size.Width());
    for (Uint32 i = 0; i < m_size.Height(); ++i) {
        m_row_pointers[i] = reinterpret_cast<Uint8*>(&m_pixels[i * m_size.Width()]);
    }
